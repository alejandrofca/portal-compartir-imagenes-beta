...
ctrl.index = async(req, res) => {
    ...
    const image_obj = await Image.findOne({ filename: { $regex: req.params.image_id } })
    if (image_obj) {
        image_obj.views = image_obj.views + 1
    ...
    const saveImage = async() => {
        ...
        if (images.length > 0) {
            ...
            if (ext === '.png' || ext === '.jpg' || ext === '.jpeg' || ext === '.gif') {
                ...
                res.status(500).json({ error: 'Solo son permitidos archivos de imagenes (png, jpg, gif)' })
            }
        }
    }
